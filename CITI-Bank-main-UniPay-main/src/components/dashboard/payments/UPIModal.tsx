...
const UPIModal: React.FC<UPIModalProps> = ({ open, onClose }) => {
  ...
  // New function to open UPI app or fallback gracefully
  const openUpi = async (appPackage?: string) => {
    if (!isValidAmount()) {
      alert("Enter a valid amount greater than 0");
      return;
    ...
    try {
      ...
      if (!appPackage) {
        // Open generic UPI URL (chooser will appear)
        window.location.href = upiUrl;
      } else {
        // Try intent URL (open specific app)
        const params = `pa=${encodeURIComponent(MERCHANT_UPI_ID)}&pn=${encodeURIComponent(MERCHANT_NAME)}&tn=${encodeURIComponent(
          `Order ${txn}`
        )}&am=${encodeURIComponent(Number(Number(amount).toFixed(2)).toString())}&cu=INR&tr=${encodeURIComponent(txn)}`;

        const intentUri = `intent://upi/pay?${params}#Intent;scheme=upi;package=${appPackage};S.browser_fallback_url=https%3A%2F%2Fplay.google.com%2Fstore/apps/details?id=${appPackage};end`;

        // This will open the intent, or fallback to Play Store if app not installed
        window.location.href = intentUri;
      }

      // Log the attempt (fire & forget)
      fetch(`${API_BASE}/api/log-intent`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
